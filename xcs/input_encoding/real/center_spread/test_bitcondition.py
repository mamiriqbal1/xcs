...
class TestBitCondition(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.condition = BitConditionRealEncoding(encoders=self.real_translator, center_spreads=[(3, 2), (5, 1)], mutation_strength=.1, mutation_prob=.2)

    def test_init(self):
        for _ in range(10):
            condition = BitConditionRealEncoding.random(self.real_translator, mutation_strength=.1, mutation_prob=.2, length=2)
            comp_array = [
                condition.clip_center_spread((center, spread), self.real_translator) == (center, spread)
    ...
    def test_matched(self):
        for _ in range(10):
            condition = BitConditionRealEncoding.random(self.real_translator, mutation_strength=.1, mutation_prob=.2, length=randint(2, 20))
            # situation(s)
            reals = [self.real_translator.clip(random_in(center - spread, center + spread)) for center, spread in condition.center_spreads]
        ...
        a_condition = BitConditionRealEncoding(encoders=self.real_translator, center_spreads=[(3, 1), (5, 0)], mutation_prob=.2, mutation_strength=.1)
    ...
    def test_not_matched(self):
        ...
        # condition(s)
        a_condition = BitConditionRealEncoding(encoders=self.real_translator, center_spreads=[(1, 1), (6, 2)], mutation_prob=.2, mutation_strength=.1)
    ...
    def test_crossover(self):
        ...
        for a_length in [2, 3] + sample(range(4, 50), 25):
            cond1 = BitConditionRealEncoding.random(encoders=a_translator, mutation_strength=.1, mutation_prob=.2, length=a_length)
            cond2 = BitConditionRealEncoding.random(encoders=a_translator, mutation_strength=.1, mutation_prob=.2, length=a_length)
            for cross_points in ([1] if a_length == 2 else [1, 2]):
                ...
                for kid in [kid1, kid2]:
                    for i, (center, spread) in enumerate(kid):
                        self.assertTrue((cond1[i][0] == center) or (cond2[i][0] == center))
                        self.assertTrue((cond1[i][1] == spread) or (cond2[i][1] == spread))
